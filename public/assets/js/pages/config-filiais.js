(function () {
  ...
  async function fetchJson(url, opts) {
    ...
    // OK, mas pode não ser JSON (ex.: HTML). Se falhar, estoura e cai no catch do loadGrid.
    return res.json();
  }

  async function loadPaises() {
    try {
      if (!routes.paises) return;
  ...
  async function loadEstados(paisId) {
    ...
    if (!paisId || !routes.estados) return;
  ...
  async function loadCidades(estadoId) {
    ...
    if (!estadoId || !routes.cidades) return;
  ...
  async function loadGrid() {
    ...
    try {
      if (!routes.grid) {
        throw new Error("Rota da grid não definida (FILIAIS_ROUTES.grid).");
      }
  ...
  async function onDelete(id) {
    ...
    swal({
      ...
      try {
        if (!routes.destroy) throw new Error("Rota de exclusão não definida.");
  ...
  function onEdit(id) {
    if (!id) return;
    const url = typeof routes.edit === "function" ? routes.edit(id) : null;
    if (!url) return;
  ...
  function initEvents() {
    ...
    // Delegação — pega clique no <a> ou no <svg> do feather
  ...
  async function init() {
    // Se o JS estiver carregando antes do HTML por algum motivo, não quebra:
    if (!els.tbody) return;
  ...
  // ✅ garante init mesmo se o script já carregar com DOM pronto
